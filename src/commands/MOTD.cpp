...
void MOTD(IRStream &stream, string &args, User *user) {
	...
	// Start of MOTD
	stream.prefix().code(RPL_MOTDSTART).param(user->getNickname()).trail(":- " + server.getHostname() + " Message of the day -").end();
	...
	stream.prefix().code(RPL_MOTD).param(user->getNickname()).trail(":- Welcome to the Interstellar Relay Chat Network!").end();
	stream.prefix().code(RPL_MOTD).param(user->getNickname()).trail(":- Your gateway to the digital cosmos: " + server.getHostname()).end();
	stream.prefix().code(RPL_MOTD).param(user->getNickname()).trail(":- ").end();
	stream.prefix().code(RPL_MOTD).param(user->getNickname()).trail(":- 🌟 Connect with beings across the universe").end();
	stream.prefix().code(RPL_MOTD).param(user->getNickname()).trail(":- 🚀 Explore channels at lightspeed").end();
	stream.prefix().code(RPL_MOTD).param(user->getNickname()).trail(":- 🌌 Share ideas that transcend space and time").end();
	stream.prefix().code(RPL_MOTD).param(user->getNickname()).trail(":- ").end();
